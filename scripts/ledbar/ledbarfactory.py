...
class LedBarFactory():
    ...
    def create_auto_led_bar_client(self, auto_update_span = 0.1):
        led_bar = self.create_led_bar_client()        
        return AutoLedBar(led_bar, auto_update_span, self.printer)
        return led_bar

    def create_led_bar_piano_client(self, midi_filename, time_scale = 1.0, left_hand_color = (10, 10, 255, 0), right_hand_color = (255, 10, 10, 0)):
        auto_led_bar = self.create_auto_led_bar_client()
        midi_file = MidiFile(midi_filename)
        led_bar_piano = LedBarPiano(midi_file, auto_led_bar, time_scale, self.printer, left_hand_color, right_hand_color)
